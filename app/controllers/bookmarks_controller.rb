class BookmarksController < ApplicationController
  before_action :current_user_must_be_bookmark_owner, only: [:edit, :update, :destroy] 
  ...
  def index
    ...
    @bookmarks = @q.result(:distinct => true).includes(:collection, :owner).page(params[:page]).per(10)
  end
  ...
  def current_user_must_be_bookmark_owner
    set_bookmark
    unless current_user == @bookmark.owner
